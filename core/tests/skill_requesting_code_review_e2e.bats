...
# End-to-end test for skills/requesting-code-review.
#
# Stages a fixture dossier (diff_base_ref: HEAD, diff_head_ref: HEAD,
# scope_paths: [core/]) so the diff is empty by construction. Invokes
# the skill's run.sh and runs chantier validate-task against the
# resulting task. All five ADR 0001 validation gates must pass, AND the
# empty-diff business case must record diff_file_count: 0 in output.json
# with run.sh still exiting 0 (D-04 + Pitfall 10 enforcement).
#
...
# chantier binary, and the make_plan helper. Like that test, this one
# initialises TMPHOME as a git repo with a single empty commit so HEAD
# resolves to something git diff can address.
...
setup() {
    ...
    # The skill's run.sh calls `git diff "$BASE...$HEAD" -- $paths`, which
    # requires TMPHOME to be a git repo with at least one commit so HEAD
    # resolves. Initialise one with a single empty seed commit.
    ...
    (
        ...
        # Empty seed commit -- HEAD needs a resolvable target.
...
@test "requesting-code-review: empty-diff end-to-end through chantier validate-task" {
    TASK="t1"
    ...
    ACC_BULLET_1="The review request cited an explicit base...head ref range and at least one path filter."
    ACC_BULLET_2="A review_prompt.md artifact was emitted at the recorded path."
    ...
    [ -f "$TASK_DIR/review_prompt.md" ]

    # output.json must be parseable JSON with all required fields.
    run jq -e '.diff_base_ref | type == "string"' "$TASK_DIR/output.json"
    [ "$status" -eq 0 ]
    run jq -e '.diff_head_ref | type == "string"' "$TASK_DIR/output.json"
    [ "$status" -eq 0 ]
    run jq -e '.diff_file_count | type == "number"' "$TASK_DIR/output.json"
    [ "$status" -eq 0 ]
    # The empty-diff business case: HEAD...HEAD must record 0 files.
    _count=$(jq -r '.diff_file_count' "$TASK_DIR/output.json")
    [ "$_count" -eq 0 ]
    # review_prompt_path must reference the actual file we asserted above.
    _path=$(jq -r '.review_prompt_path' "$TASK_DIR/output.json")
    [ -f "$_path" ]
